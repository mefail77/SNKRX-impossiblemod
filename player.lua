...
function Player:init(args)
  ...
  elseif self.character == 'magician' then
    self.attack_sensor = Circle(self.x, self.y, 96)
    self.t:cooldown(0.01, function() local enemies = self:get_objects_in_shape(self.attack_sensor, main.current.enemies); return enemies and #enemies > 0 end, function()
  ...
  elseif self.character == 'silencer' then
    ...
    self.t:cooldown(6, function() local enemies = self:get_objects_in_shape(self.attack_sensor, main.current.enemies); return enemies and #enemies > 0 end, function()
      ...
      curse()
      if main.current.sorcerer_level > 0 then
        self.sorcerer_count = self.sorcerer_count + 69
        if self.sorcerer_count >= ((main.current.sorcerer_level == 3 and 2) or (main.current.sorcerer_level == 2 and 3) or (main.current.sorcerer_level == 1 and 4)) then
          self.sorcerer_count = 0
          self:sorcerer_repeat()
          self.t:after(0.5, function()
  ...
  elseif self.character == 'barrager' then
    ...
    self.attack_sensor = Circle(self.x, self.y, 128)
    self.t:cooldown(0.1, function() local enemies = self:get_objects_in_shape(self.attack_sensor, main.current.enemies); return enemies and #enemies > 0 end, function()
      local closest_enemy = self:get_closest_object_in_shape(self.attack_sensor, main.current.enemies)
  ...
  elseif self.character == 'highlander' then
    ...
    self.t:cooldown(60, function() local enemies = self:get_objects_in_shape(self.attack_sensor, main.current.enemies); return enemies and #enemies > 0 end, function()
      if self.level == 3 then
...
function Player:update(dt)
  ...
  if table.any(self.classes, function(v) return v == 'cultist' end) then
    ...
    elseif main.current.cultist_level == 0 then self.cultist_stat_mult = 0
...
function Projectile:on_collision_enter(other, contact)
  ...
  if other:is(Wall) then
    ...
    elseif self.character == 'engineer' or self.character == 'dual_gunner' or self.character == 'miner' then
      self:die(x, y, r, random:int(2, 3))
      _G[random:table{'turret_hit_wall1', 'turret_hit_wall2'}]:play{pitch = random:float(0.9, 1.1), volume = 0.2} --  or self.character == 'witch'
    elseif self.character == 'psyker' then
    else
      self:die(x, y, r, random:int(2, 3))
...
function Projectile:on_trigger_enter(other, contact)
  ...
  if table.any(main.current.enemies, function(v) return other:is(v) end) then
	...
	if self.character == 'rocketeer' then
		if (self.crit) and (self.level == 3) then
			Area{group = main.current.effects, x = self.x, y = self.y, r = self.r, w = self.parent.area_size_m*72, color = self.color, dmg = self.parent.area_dmg_m*self.dmg, character = self.character, parent = self,
        void_rift = self.parent.void_rift, echo_barrage = self.parent.echo_barrage}
		else
			Area{group = main.current.effects, x = self.x, y = self.y, r = self.r, w = self.parent.area_size_m*48, color = self.color, dmg = self.parent.area_dmg_m*self.dmg, character = self.character, parent = self,
        void_rift = self.parent.void_rift, echo_barrage = self.parent.echo_barrage}
		end
	end
...
function Area:init(args)
  ...
  for _, enemy in ipairs(enemies) do
    ...
    elseif self.character == 'freezing_field' then
      enemy:slow(0.5, 2)
	elseif self.character == 'healing_aoe' then
    else
    ...
    if self.chsracter == 'healing_aoe' then
		local enemies = main.current.main:get_objects_in_shape(self.shape, main.current.enemies)
    end
  end
...
function DotArea:init(args)
  ...
  if self.character == 'plague_doctor' or self.character == 'pyromancer' or self.character == 'witch' or self.character == 'burning_field' or self.character == 'mortar' or self.character == 'acolyte_dot' then
...
function HealingOrb:init(args)
  ...
  if not self.healing_power then
...
function HealingOrb:draw()
  ...
  if (self.heal_aoe) then
	graphics.push(self.x, self.y, self.r, self.hfx.hit.x + sr, self.hfx.hit.x + sr)
		graphics.circle(self.x, self.y, 1.2*self.shape.w, self.hfx.hit.f and fg[0] or red_transparent_weak)
		graphics.circle(self.x, self.y, 0.5*self.shape.w, self.hfx.hit.f and fg[0] or red[0])
	graphics.pop()
  else
	graphics.push(self.x, self.y, self.r, self.hfx.hit.x + sr, self.hfx.hit.x + sr)
...
function HealingOrb:on_trigger_enter(other, contact)
  ...
  if other:is(Player) then
    ...
    if self.heal_aoe then
		local t = {group = main.current.effects, x = lowest_unit.x, y = lowest_unit.y, r = self.r, w = 24, color = self.color, dmg = 5,
			character = 'healing_aoe', level = self.parent.level, parent = self}
		Area(t)
		if lowest_unit then
